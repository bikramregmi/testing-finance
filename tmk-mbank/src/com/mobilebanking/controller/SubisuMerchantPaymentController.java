...
@Controller
public class SubisuMerchantPaymentController {
	...
	@RequestMapping(method = RequestMethod.GET, value = "/subisu")
	public String merchantPayment(ModelMap modelMap, HttpServletRequest request) {
		String authority = AuthenticationUtil.getCurrentUser()
				.getAuthority();
		if (authority.contains(Authorities.AGENT) || 
				authority.contains(Authorities.BANK) || authority.contains(Authorities.BANK_ADMIN)|| 
				authority.contains(Authorities.BANK_BRANCH_ADMIN) || 
				authority.contains(Authorities.BANK_BRANCH)) {
			MerchantServiceDTO merchantService = merchantServiceApi.findServiceByUniqueIdentifier(StringConstants.SUBISU);
			
			modelMap.put("serviceDTO", merchantService);
			
			return "MerchantPayment/TopSup";
		}
		return "redirect:/";
	}

	@RequestMapping(method = RequestMethod.POST, value = "/subishuPayment")
...
HashMap<String, String> validationResponse = merchantPaymentValidation.merchantPaymentValidation(transactionId, serviceId, Double.parseDouble(amount), AuthenticationUtil.getCurrentUser().getId());

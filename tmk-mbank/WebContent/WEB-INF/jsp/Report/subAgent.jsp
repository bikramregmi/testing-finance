...
<spr:page header1="Sub-Agent Wise Report">
		<div class="break"></div>
		<form  action="subAgentReport" modelAttribute="menu" method="post" class="col-md-offset-4 col-md-4" style="float: none; margin-left: auto; margin-right: auto;">
			<div class="form-group">
				<label>Sub-Agent Name</label>
				<c:choose>
					<c:when test="${empty subAgentList}">
					...
					<c:otherwise>
						<select name="id" class="form-control input-sm" required="required">
							<c:forEach var="agent" items="${subAgentList}">
								<c:choose>
									<c:when test="${search.id == agent.id}">
										<option value="${agent.id}" selected>${agent.firstName} ${agent.middleName} ${agent.lastName} | ${agent.mobileNo}</option>
									</c:when>
									<c:otherwise>
										<option value="${agent.id}">${agent.firstName} ${agent.middleName} ${agent.lastName} | ${agent.mobileNo}</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>

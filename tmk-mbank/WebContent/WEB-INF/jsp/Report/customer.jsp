...
<spr:page header1="Customer Wise Report">
		<div class="break"></div>
		<form  action="customerReport" modelAttribute="menu" method="post" class="col-md-offset-4 col-md-4" style="float: none; margin-left: auto; margin-right: auto;">
					...
					<c:otherwise>
						<select name="id" class="form-control input-sm" required="required">
							<c:forEach var="customer" items="${customerList}">
								<c:choose>
									<c:when test="${search.id == customer.id}">
										<option value="${customer.id}" selected>${customer.firstName} ${customer.middleName} ${customer.lastName} | ${customer.mobileNo}</option>
									</c:when>
									<c:otherwise>
										<option value="${customer.id}">${customer.firstName} ${customer.middleName} ${customer.lastName} | ${customer.mobileNo}</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>

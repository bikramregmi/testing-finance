...
<spr:page header1="Assign Secondary Currency">
	<div class="col-md-12">
		<div class="break"></div>
		...
		<form action="addSecondaryCurrency" method="post"
			...
			<div class="row">
				<div class="col-sm-5">
					<select class="form-control pickListSelect" id="pickData"
						...
						<c:forEach items="${unselectedCurrencyList}" var="element">
							<option id="${element.id}">${element.currencyName}</option>
						</c:forEach>
					</select>
				</div>
				...
				<div class="col-sm-5">
					<select class="form-control pickListSelect" id="pickListResult"
						...
						<c:forEach items="${selectedCurrencyList}" var="element">
							<option id="${element.id}">${element.currencyName}</option>
						</c:forEach>
					</select>
				</div>
			...
			<br /> <br /> <input type="hidden" id="ids" name="ids"
				value="<c:forEach items="${selectedCurrencyList}" var="element" varStatus="loop">
